...
class DepthEstimationUNetResNet34(pl.LightningModule):
    def __init__(
        ...
        self.upconv4 = self.pixel_shuffle_block(512, 256)  # 256 * 4 = 1024 output channels
        self.decoder4 = self.decoder_block(256 + 256, 256)  # 256 from upconv4 + 256 from encoder3

        self.upconv3 = self.pixel_shuffle_block(256, 128)   # 128 * 4 = 512 output channels
        self.decoder3 = self.decoder_block(128 + 128, 128)  # 128 from upconv3 + 128 from encoder2

        self.upconv2 = self.pixel_shuffle_block(128, 64)    # 64 * 4 = 256 output channels
        self.decoder2 = self.decoder_block(64 + 64, 64)     # 64 from upconv2 + 64 from encoder1

        self.upconv1 = self.pixel_shuffle_block(64, 16)     # 16 * 4 = 64 output channels
        self.decoder1 = self.decoder_block(16 + 64, 16)     # 16 from upconv1 + 64 from initial
    ...
    def forward(self, x):
        ...
        d4 = torch.cat([d4, x3], dim=1)             # [B, 256 + 256 = 512, H/16, W/16]
        d4 = self.decoder4(d4)                      # [B, 256, H/16, W/16]

        d3 = self.upconv3(d4)                       # [B, 128, H/8, W/8]
        d3 = torch.cat([d3, x2], dim=1)             # [B, 128 + 128 = 256, H/8, W/8]
        d3 = self.decoder3(d3)                      # [B, 128, H/8, W/8]

        d2 = self.upconv2(d3)                       # [B, 64, H/4, W/4]
        d2 = torch.cat([d2, x1], dim=1)             # [B, 64 + 64 = 128, H/4, W/4]
        d2 = self.decoder2(d2)                      # [B, 64, H/4, W/4]

        d1 = self.upconv1(d2)                       # [B, 16, H/2, W/2]

        x0_up = F.interpolate(x0, size=d1.shape[2:], mode='bilinear',
                              align_corners=False)  # [B, 64, H/2, W/2]

        d1 = torch.cat([d1, x0_up], dim=1)           # [B, 16 + 64 = 80, H/2, W/2]
        d1 = self.decoder1(d1)                      # [B, 16, H/2, W/2]
    ...
    def compute_smoothness_loss(self, preds):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = self.criterion(preds, depths) + self.compute_smoothness_loss(preds)
